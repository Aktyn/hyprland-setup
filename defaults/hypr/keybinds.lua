hl.define_submap("global", function()
	...
	local player_next = 'playerctl next || playerctl position `bc <<< "100 * $(playerctl metadata mpris:length) / 1000000 / 100"`'
...
-- 	hl.bind("SUPER + catchall", hl.dsp.global("quickshell:overviewToggleReleaseInterrupt")) -- Original bind: Super, catchall, global, quickshell:overviewToggleReleaseInterrupt
	...
	hl.bind(
		"SUPER + T",
		hl.dsp.exec_cmd(
			launch_first_available .. ' "$TERMINAL" "kitty -1" "foot" "alacritty" "wezterm" "konsole" "kgx" "uxterm" "xterm"'
		)
	)
	hl.bind(
		"CTRL + ALT + T",
	...
	hl.bind(
		"SUPER + W",
		hl.dsp.exec_cmd(
			launch_first_available .. ' "google-chrome-stable" "brave" "zen-browser" "firefox" "chromium" "microsoft-edge-stable" "opera" "librewolf"'
		)
	)
	hl.bind(
		"SUPER + C",
		hl.dsp.exec_cmd(
			launch_first_available .. ' "code" "codium" "cursor" "zed" "zedit" "zeditor" "kate" "gnome-text-editor" "emacs" "command -v nvim && kitty -1 nvim"'
		)
	)
	hl.bind(
		"SUPER + X",
	...
	hl.bind(
		"SUPER + I",
		hl.dsp.exec_cmd(
			'XDG_CURRENT_DESKTOP=gnome ' .. launch_first_available .. ' "systemsettings" "gnome-control-center" "better-control"'
		)
	)
	hl.bind(
		"CTRL + SHIFT + ESCAPE",
		hl.dsp.exec_cmd(
			launch_first_available .. ' "gnome-system-monitor" "plasma-systemmonitor --page-name Processes" "command -v btop && kitty -1 fish -c btop"'
		)
	)
