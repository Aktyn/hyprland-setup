...
def update_hyprland_colors(colors):
    dynamic_lua_path = f"{os.path.expanduser('~')}/.config/hypr/dynamic.lua"

    try:
        with open(dynamic_lua_path, 'w') as f:
            f.write(f"-- The values below are autogenerated. Do not change them manually.\n"
                    f"hl.config({{\n"
                    f"    general = {{\n"
                    f"        ['col.active_border'] = '{hex_to_rgba(colors['primary'], 0.5)}',\n"
                    f"        ['col.inactive_border'] = '{hex_to_rgba(colors['outline'], 0.25)}'\n"
                    f"    }},\n"
                    f"    misc = {{\n"
